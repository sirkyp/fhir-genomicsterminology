...
class Cytoband:
  ...
  def process_data(self):
    ...
    cytobandCS = FHIRCodeSystem()
    cytobandCS.init(name = "Cytoband",title = "Chromosome Bands Localized by FISH Mapping Clones",url = "http://hl7.org/fhir/uv/molecular-definition-data-types/CodeSystem/cytoband", version = "1.0.0", experimental = True, publisher = "UCSC Genome Browser", description = "", 
                    contact=[{"name": "UCSC Genome Browser", "telecom": [{"system": "url", "value": "https://genome.ucsc.edu/"}]}],
                    relatedArtifact=[{"type": "documentation", "label": "UCSC Cytoband Track", "url": "https://genome.ucsc.edu/cgi-bin/hgTables?hgsid=2635207210_psoFu973UsC7wWlBbsqfQvoIWWYo&clade=mammal&org=Human&db=hg38&hgta_group=map&hgta_track=cytoBand&hgta_table=0&hgta_regionType=genome&position=chr7%3A155%2C799%2C529-155%2C812%2C871&hgta_outputType=primaryTable&hgta_outFileName="}])
...
We also remove bands with 'alt', 'fix', 'Un_', '_random' in the name, as these are not standard cytobands.
    ...
    cytobandCS.add_property(code="start", type=FHIRCodeSystem.PropertyType.INTEGER, description="Chromosome Start (0-based)")
    cytobandCS.add_property(code="end", type=FHIRCodeSystem.PropertyType.INTEGER, description="Chromosome End (0-based)")
    cytobandCS.add_property(code="giestain", type=FHIRCodeSystem.PropertyType.STRING, description="gieStain")
    ...
    with open(LOCAL_DATA_FILE, 'r') as file:
        ...
        for a in reader:
            ...
            if 'alt' in code or 'fix' in code or 'Un_' in code or '_random' in code:
                ...
                continue
            c = cytobandCS.add_concept(code=code, display=code, definition="")
        ...
        for c in range(1, 24):
          cytobandCS.add_concept(code=f"{c}p", display=f"{c}p", definition="")
          cytobandCS.add_concept(code=f"{c}q", display=f"{c}q", definition="")
          
        cytobandCS.add_concept(code="Xp", display="Xp", definition="")
        cytobandCS.add_concept(code="Xq", display="Xq", definition="")
        cytobandCS.add_concept(code="Yp", display="Yp", definition="")
        cytobandCS.add_concept(code="Yq", display="Yq", definition="")
